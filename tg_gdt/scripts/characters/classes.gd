...
func generate_class_stats(class_ranges: Dictionary) -> Dictionary:
	var stats = {}
	# For each stat defined in the stat_ranges, choose a random integer between the two values (inclusive)
	for stat in class_ranges.keys():
		var range_vals = class_ranges[stat]
		stats[stat] = randi() % (range_vals[1] - range_vals[0] + 1) + range_vals[0]
	return stats
